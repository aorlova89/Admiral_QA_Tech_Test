import 'mocha';
import {LocationAreasResponse, LocationResponse} from "../src/models/LocationResponses.model";
...
describe('Location endpoint tests', () => {
    ...
    it('Location name is a string', () => {
        return chai.request(url).get('1').then((res: LocationResponse) => {
    ...
    it('Location area url is correct', () => {
        return chai.request(url).get('1').then((res: LocationResponse) => {
            ...
            chai.request(res.body.areas[0].url).get('').then((resp: LocationAreasResponse) => {
...
describe('Location Area endpoint tests', () => {
    ...
    it('Location-area response has all expected keys', () => {
        return chai.request(url).get('1').then((res: LocationAreasResponse) => {
