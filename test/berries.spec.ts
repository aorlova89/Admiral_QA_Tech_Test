import 'mocha';
import {BerryFlavorResponse, BerryResponse} from "../src/models/BerriesResponses.model";
...
describe('Berries endpoint tests', () => {
    ...
    it('Berry #10 has flavor Bitter with potency = 10', () => {
        return chai.request(url).get('10').then((res: BerryResponse) => {
            const flavors = res.body.flavors;
            const bitter = flavors.find((el => el.flavor.name==='bitter'));
            chai.expect(bitter?.potency, 'Berry id=10 should have Bitter flavor with potency=10').equal(10);
        });
    });
});
...
describe('Berry Flavors endpoint tests', () => {
    ...
    it('Flavor with id=4 is Bitter', () => {
        return chai.request(url).get('4').then((res: BerryFlavorResponse) => {
