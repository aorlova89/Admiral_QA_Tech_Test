...
describe("Pokemon endpoint tests", () => {
  ...
  it("Pokemon number 100 name should be voltorb", () => {
    return chai
      .request(url)
      .get("100/")
      .then((res: PokemonResponses) => {
        chai.expect(res).to.have.status(200);
        chai
          .expect(res.body.name, "Expected name for id=100 is voltorb")
          .to.eql("voltorb");
      });
  });
  it("Pokemon number 100 name has sprites", () => {
    return chai
      ...
      .then((res: PokemonResponses) => {
        chai.expect(res).to.have.status(200);
        chai.expect(res.body.sprites, "There should be sprites for id=100").to
          .not.be.null;
      });
  });
      ...
      .then((res: PokemonResponses) => {
        ...
        chai
          .expect(
            ...
            "Pokemon with id=100 should have Electric type"
          )
          .to.eql("electric");
      });
  });
    ...
    return chai
      ...
      .then((res: PokemonResponses) => {
        ...
        chai
          .expect(
            hiddenAbilities,
            "Pokemon id=100 should have 1 hidden ability"
          )
          .to.equal(1);
        chai
          .expect(
            visibleAbilities,
            "Pokemon id=100 should have 2 visible abilities"
          )
          .to.equal(2);
      });
  });
